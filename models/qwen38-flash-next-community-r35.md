# Qwen3.8-Flash-Next — community R35 deployment and measurement archive

Historical record: commands and qualification below belong to the identified
community images. For the profile-based GHCR image, use the
[Qwen model page](qwen38-flash-next.md) and
...
Download the [Compose file](qwen38-flash-next/qwen38-flash-next.community-r35.compose.yml):

```bash
curl -fL -o qwen38-flash-next.community-r35.compose.yml \
  https://raw.githubusercontent.com/local-inference-lab/rtx6kpro/master/models/qwen38-flash-next/qwen38-flash-next.community-r35.compose.yml

GPU=0 PORT=8000 docker compose -f qwen38-flash-next.community-r35.compose.yml --profile tp1 up -d
docker compose -f qwen38-flash-next.community-r35.compose.yml --profile tp1 logs -f qwen-tp1
```
...
```bash
docker compose -f qwen38-flash-next.community-r35.compose.yml --profile tp1 down
GPU0=0 GPU1=1 PORT=8000 docker compose -f qwen38-flash-next.community-r35.compose.yml --profile tp2 up -d
docker compose -f qwen38-flash-next.community-r35.compose.yml --profile tp2 logs -f qwen-tp2
```
...
VLLM_PLE_CPU_OFFLOAD=0 GPU0=0 GPU1=1 PORT=8000 \
  docker compose -f qwen38-flash-next.community-r35.compose.yml --profile tp2 up -d
```
