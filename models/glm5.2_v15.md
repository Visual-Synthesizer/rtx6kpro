# GLM-5.2 v15 NVFP4 / Online FP8-MXFP8 Overlay
...
No client is started while another model is still loading. The existing v14
container on GPUs `8-15` was left running and was not touched.
...
```text
scripts/build-glm52-v15-final-image.sh
```
...
| KV cache | `fp8` |
| Load format | `instanttensor`, `INSTANTTENSOR_BACKEND=BUFFERED` |
| f8 DMA | `0` |
...
The final image contains the GLM 5.2 v15 launcher:

```text
/usr/local/bin/run-glm52-v15-server
```
...
services:
  server:
    ...
    volumes:
      ...
      - ${CACHE_DIR:-/root/.cache/vllm-glm52-v15/glm52-v15}:/cache:rw
      - ${TMP_DIR:-/root/vllm/tmp/glm52-v15}:/container-tmp:rw
    command:
      - run-glm52-v15-server
```
...
| `IMAGE` | final v15 image | Docker image to run. |
...
```text
compose/glm52-v15.yml
scripts/run-glm52-v15-compose.sh
```
...
The standalone compose example above can be run directly because it includes
`gpus: all`. The checked-in `compose/glm52-v15.yml` is meant to be run through
the helper below; the helper injects explicit Docker GPU `DeviceRequests` for
the selected `GPUS` list so it does not expose all 16 cards by accident.

```bash
cd /root/rtx6kpro
NAME=glm52-v15-online-a16 PORT=8000 GPUS=0,1,2,3,4,5,6,7 \
MOE_MODE=a16 ONLINE_MXFP8=1 MTP=3 MAX_NUM_SEQS=32 \
./scripts/run-glm52-v15-compose.sh up
```
...
Run the reduced v15 validation sweep used for this page:

```bash
cd /root/rtx6kpro
RESULT_ROOT=/root/bench-results/glm52-v15-final-reduced-$(date -u +%Y%m%dT%H%M%SZ) \
PORT=8001 \
...
Run only the A4 online MXFP8 sanity rerun if one standalone prefill sub-run
looks like an outlier:

```bash
cd /root/rtx6kpro
RESULT_ROOT=/root/bench-results/glm52-v15-rerun-a4-online-8konly-$(date -u +%Y%m%dT%H%M%SZ) \
CASES='nvfp4-a4-online-mxfp8' \
RUN_MTP3=0 \
PREFILL_CONTEXTS=8k \
PORT=8001 \
GPUS=0,1,2,3,4,5,6,7 \
...
The v15 reduced helper writes progress to `${RESULT_ROOT}/progress.log` by
default. The main run in this page used:

```text
/root/bench-results/glm52-v15-final-reduced-20260709T024110Z/progress.log
```

Summary command for the JSON artifacts:

```bash
python3 - <<'PY'
import json, pathlib
root = pathlib.Path("/root/bench-results/glm52-v15-final-reduced-20260709T024110Z")
for case in sorted(root.glob("*/*")):
    dec = case / "decode.json"
    pre = case / "prefill.json"
    kv = case / "kv_cache_summary.json"
    if not dec.exists():
        continue
    d = json.load(open(dec))
    p = json.load(open(pre))["prefill"] if pre.exists() else {}
    k = json.load(open(kv)).get("gpu_kv_cache_tokens") if kv.exists() else ""
    print(case, d["results"][0]["aggregate_tps"], p.get("8192", {}).get("tok_per_sec", ""), p.get("65536", {}).get("tok_per_sec", ""), k)
PY
```
