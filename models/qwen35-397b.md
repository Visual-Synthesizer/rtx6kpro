...
### Recommended: NVFP4, 4x GPUs, b12x + MTP (180 tok/s single user)
...
Without MTP (remove the `--speculative-*` lines and `SGLANG_ENABLE_SPEC_V2`): **108 tok/s** single user, **1550 tok/s** at 64 concurrent.
...
### Decode throughput — 4x RTX PRO 6000, TP=4, `voipmonitor/sglang:cu130` (b12x 0.7.2)

**SGLang b12x MoE + flashinfer attention + PCIe oneshot + MTP** (April 2026):
...
**SGLang b12x MoE + flashinfer attention + PCIe oneshot, no MTP** (April 2026):
...
**Summary — Best Configuration by Scenario:**

| Scenario | Best Setup | tok/s |
|----------|-----------|-------|
| Single user, max speed | SGLang b12x + MTP | **180 tok/s** |
| Single user, no MTP | SGLang b12x + PCIe oneshot | **108 tok/s** |
| Multi-user (c=32) | SGLang b12x, no MTP | **1174 tok/s** |
| Multi-user (c=64) | SGLang b12x, no MTP | **1551 tok/s** |
| High concurrency (c=128) | vLLM | **2277 tok/s** |

### Single-stream decode throughput summary

| Config | Engine | MTP | tok/s | Notes |
|--------|--------|-----|-------|-------|
| **397B NVFP4** | **SGLang b12x + PCIe oneshot** | **MTP** | **180.4** | **b12x 0.7.2, April 2026** |
| 397B NVFP4 | SGLang b12x + PCIe oneshot | No | 108.1 | b12x 0.7.2, April 2026 |
