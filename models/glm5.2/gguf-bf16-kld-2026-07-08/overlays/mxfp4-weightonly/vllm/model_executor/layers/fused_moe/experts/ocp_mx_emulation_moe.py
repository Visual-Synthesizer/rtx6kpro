...
def _dequant_mxfp4_torch(
    ...
    dequant = values[unpacked.to(torch.long)].to(float_dtype)
    dequant = dequant.reshape(*dequant.shape[:-1], -1, 32)
    scale_float = torch.exp2(scale.to(torch.float32) - 127.0).to(float_dtype)
    dequant = dequant * scale_float[..., None]
    return dequant.reshape(*dequant.shape[:-2], -1)
...
class OCP_MXQuantizationEmulationTritonExperts(TritonExperts):
    ...
    def __init__(
        ...
        if self.ocp_mx_scheme in (OCP_MX_Scheme.w_mxfp4, "w_mxfp4"):
            # Weight-only MXFP4 diagnostic: dequantize weights to BF16 but do
            # not quantize/dequantize activations in TritonExperts.apply.
    ...
    def _dequantize_weights(
        ...
        if self.ocp_mx_scheme.startswith("w_mxfp4"):  # type: ignore[union-attr]
            return _dequant_mxfp4_torch(w, w_scale, dtype)
