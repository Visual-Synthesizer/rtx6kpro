...
NAME_A=glm52-v18-validation-a
NAME_B=glm52-v18-validation-b
...
[[ "$(docker image inspect "${IMAGE}" --format '{{.Id}}')" == "${EXPECTED_IMAGE_ID}" ]] || {
  echo "image ID does not match the validated v18 image" >&2
  exit 2
}
...
start_case() {
  ...
  served="GLM-5.2-v18-$(safe_name "${key}-tp${tp}-dcp${dcp}-mtp${mtp}")"
...
if tp == 8 and dcp in (4, 8):
    assert summary["fast_dcp_path"], "v18 fast DCP path was not observed"
if mtp:
    assert (
